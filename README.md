# auxillary-projects

Darey.io Auxilllary Project

## How to Onboard Linux users onto a server

- Create an EC2 Ubuntu Instance in AWS
- Connect remotely to EC2
- make a Shell Directory and Navigate into the directory

```
mkdir Shell && cd Shell
```

- create the following files

1. names.csv - Containing a List of Linux users to be onboarded
1. id_rsa - Private Access Key
1. id_rsa.pub - Public Key
1. onboarding.sh - Bash Script File to automate adding of users

```
touch names.csv id_rsa id_rsa.pub onboarding.sh
```

- Using favorite editor populate datas into names.csv (One name per Line)

- open id_rsa.pub

```
nano id_rsa.pub
```

and paste this access key

```
ssh-rsa 
```

- Open Private Key

```
nano id_rsa
```

and paste this Private Access key

```
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
```

- Create developers group mannually

```
sudo groupadd developers
```

- Open onboarding.sh script and paste the following bash script

```
nano onboarding.sh
```

```
#! /bin/bash

# Read users files
userfile=$(cat names.csv)
PASSWORD=password

# Check if User has root privilege
if [ $(id -u) -eq 0 ];  then

# read the User in CSV file
    for user in $userfile;
    do
        echo $user
    if id "$user" &>/dev/null
    then
            echo $user "User Exist"
    else

        # Create a new User
        useradd -m -d /home/$user -s /bin/bash -g developers $user
        echo $user "Created Successfuly"
        echo


        # Creat ssh folder in the user home directory
        su - -c "mkdir ~/.ssh" $user
        echo ".ssh directory created for " $user
        echo

        # Set user permission for the ssh directory
        su - -c "chmod 700 ~/.ssh" $user
        echo $user "user permission for .ssh directory is set"
        echo

        # Create an authorized-key file
        su - -c "touch ~/.ssh/authorized_keys" $user
        echo "Authorized Key File Created for " $user
        echo

        # Set permission for the key file
        su - -c "chmod 600 ~/.ssh/authorized_keys" $user
        echo "user permission for the Authorized Key File set"
        echo

        # Create and set public key for users in the server
        cp -R "/home/ubuntu/Shell/id_rsa.pub" "/home/$user/.ssh/authorized_keys"
        echo "Copied the Public Key to Account on the server"
        echo
        echo

        echo $user "USER is CREATED"

# Generate a password.
sudo echo -e "$PASSWORD\n$PASSWORD" | sudo passwd "$user"
sudo passwd -x 3 $user
            fi
        done
    else
    echo "Only Admin Can Onboard A User"
    fi
```

- Make the shell script Executable

```
sudo chmod +x onboarding.sh
```

- Change user to Root/Super user

```
sudo su
```

then

- Run onboarding bash script

```
./onboarding.sh
```

![](images/addusers.png)

- Verify the New User

```
cd ../..
```

```
ls -la
```

![](images/listusers.png)

## To verify you can Login to any of the created user

- Open another terminal, create a new access key and copy and paste the private key

```
touch auxilarypro.pem
```

```
nano auxillarypro.pem
```

```
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
```

- Remotely sign into any of the created user

```
ssh -i auxillarypro.pem new_user@ubutu_public_ipv4_address
```
